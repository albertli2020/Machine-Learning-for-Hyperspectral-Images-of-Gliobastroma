...
t_images, nt_images = generatePatchLists(image_dir)
image_files = dataSetSample(data_precentage, t_images, nt_images)
labels = labelHSIDataSet(image_files)
training_images, test_images, training_labels, test_labels = train_test_split(image_files, labels, test_size=0.1)

print("Total number of training data:", len(training_images))
...
dataset = HSIImageDataset(training_images, training_labels, batch_size)
...
test_dataset = HSIImageDataset(test_images, test_labels, batch_size, data_precentage)
