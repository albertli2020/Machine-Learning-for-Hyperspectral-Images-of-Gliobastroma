...
class HyperspectralDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Average all 275 bands
        patch_mean = np.mean(patch_np[:87, :87, :275], axis=2)

        # Ensure the patch is a 2D array (87, 87)
        assert patch_mean.shape == (87, 87), f"Unexpected patch size: {patch_mean.shape} at {npy_path}"

        # Convert to PyTorch tensor and add the band dimension
        patch = torch.tensor(patch_mean, dtype=torch.float32).unsqueeze(0)  # Shape: (1, 87, 87)
